...
**가격 어댑터는 끝났고(미결 9번), 다음은 채점기와 지표다**(미결 10번).
`src/eval/`와 `src/compute/`가 비어 있다. 설계는 `docs/harness.md` 4절에 확정돼 있고,
**구현 중 실측으로 정정된 두 가지**(열 이름, 초과수익 기준)가 그 문서 6절에 정리돼 있다.
...
10. **채점기와 지표** — 남은 하네스. `src/eval/`와 `src/compute/`가 아직 비어 있다.
    설계는 `docs/harness.md` 4절에 확정돼 있다. **여기가 지금 1순위 구현 대상이다**
