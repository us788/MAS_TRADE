...
**수집이 안정적으로 돌면 그다음은 가격 어댑터 → 백테스트 하네스다** (미결 9·10번).
기획서가 1순위 산출물로 꼽은 것이 백테스트 하네스이고, **설계는 `docs/harness.md`에
끝나 있다.** 구현 전에 그 문서 6절의 미정 다섯 가지를 먼저 정한다.
...
9. **가격 어댑터** (yfinance / pykrx) — 시그널 채점의 유일한 선행 조건.
   **설계는 `docs/harness.md`에 확정돼 있다.** 과거 조회가 되니 데이터 손실 위험은 없다
10. **백테스트 하네스** — 기획서가 1순위 산출물로 꼽은 것. 수집이 안정됐으니 여기로.
    설계 `docs/harness.md`. 구현 전에 그 문서 6절의 다섯 가지를 먼저 정해야 한다
