...
| | |
|---|---|
| ✅ **동작** | 뉴스·가격 수집, as-of 인터페이스, 지표, 컨텍스트, 베이스라인 단일 호출, 시그널 저장, 층 1 채점, 비교군(지수·랜덤·룰 6종), 저울 검정, launchd 2작업 |
| 🟡 **부분** | SEC EDGAR 어댑터(코드는 있으나 403), gap 자동 해소 없음 |
| ❌ **미구현** | 분석 에이전트 5종, 논쟁 레이어, **리스크 엔진**, 리플렉션, 층 2 가상 포트폴리오, DART 어댑터, 매크로(ALFRED) 어댑터, 오픈소스 프레임워크 비교 |
