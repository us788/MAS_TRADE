...
- **종목코드로 검색되지 않는다.** 회사명으로 쿼리해야 해서 동명이의 잡음이 크다
  ("한화" → 야구단 기사). `news_exclude`로 배제한다
- **본문까지 매칭하는 키워드 검색이다.** 스니펫에 검색어가 거의 항상 들어 있어
  "언급됐는가" 필터는 100% 통과한다. 실측(2026-09-15) 삼성전자 1,000건 중 제목 매칭 18%.
  → 버리지 않고 `relevance` 등급(title/summary/none)으로 저장하고 기본 파이프라인은
  `title`만 쓴다
- **최신순 1,000건이 덮는 구간이 종목마다 다르다.** 삼성전자 7시간 / 한화에어로 192시간.
  하루 1회로 통일하면 삼성전자는 매일 17시간이 빈다.
  → `collect_every_hours`로 종목별 주기 배정 (`scripts/measure_news_rate.py`)
...
| 벤더 원본 응답 | `data/snapshots/raw/{소스}/{날짜}/{종목}_{시각}.json` |

중복 제거는 `(symbol, content_hash)` PRIMARY KEY가 구조적으로 보장한다. 재수집 시
`INSERT OR IGNORE`이므로 **`collected_at`은 최초 관측 시각으로 고정된다** — `known_at`의
보수적 판정이 이 값에 기대므로 나중 값으로 덮지 않는다.

원본 응답은 **수집 1회가 파일 하나**다. 페이지별로 쪼개면 파일명이 수집 시각 하나로 같아져
서로를 덮어쓴다 (실제로 8페이지 중 1개만 남은 적이 있다).

### 수집 주기와 lookback

cron은 한 시간마다 `scripts/collect_news.py`를 깨우기만 한다. 대상 선정은
`collection_runs`의 마지막 수집 시각과 `universe.json`의 `collect_every_hours`가 정한다.

lookback은 **마지막 수집 이후 경과 시간의 1.5배**다 (하한 1시간, 상한 7일, 첫 수집은
주기의 2배). 주기 기준 고정값은 평소엔 과하게 받아 페이지 상한에 가까워지고, 장기 중단
뒤에는 그 구간을 못 메운다. 겹쳐 받는 비용은 0이다 — 중복은 DB가 막는다.
